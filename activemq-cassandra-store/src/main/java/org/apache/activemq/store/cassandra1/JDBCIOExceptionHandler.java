...
public class JDBCIOExceptionHandler extends DefaultIOExceptionHandler {
    ...
    protected boolean hasLockOwnership() throws IOException {
        ...
        if (broker.getPersistenceAdapter() instanceof CassandraPersistenceAdapter) {
            CassandraPersistenceAdapter cassandraPersistenceAdapter = (CassandraPersistenceAdapter) broker.getPersistenceAdapter();
            Locker locker = cassandraPersistenceAdapter.getLocker();
            if (locker != null) {
                try {
                    if (!locker.keepAlive()) {
                        hasLock = false;
